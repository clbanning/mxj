...
func main() {
	...
	for m, err = mxj.NewMapXmlSeqReader(rdr); m != nil || err != io.EOF; m, err = mxj.NewMapXmlSeqReader(rdr) {
		...
		for _, t := range vals {
			// get Name from attrs
			var tname string
			for _, vv := range t.(map[string]interface{})["#attr"].([]interface{}) {
				if ttname, ok := vv.(map[string]interface{})["Name"]; ok {
					tname = ttname.(string)
					break
				}
			}
			// fmt.Println(mxj.Map(t.(map[string]interface{})).StringIndent())

			// now process TransactionTimer.Items value ... is a map[string]interface{} value
			// with Comment and Request keys with array values
			vm, ok := t.(map[string]interface{})["Items"].(map[string]interface{})
			if !ok {
